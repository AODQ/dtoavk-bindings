...
// This needs to be used along with a Renderer (e.g. OpenGL3, Vulkan..) (Info:
...
//  'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange'. FIXME: 3 cursors
//  types are missing from GLFW.
//  [X] Platform: Keyboard arrays indexed using GLFW_KEY_* codes, e.g.
...
// About GLSL version: The 'glsl_version' initialization parameter defaults to
// "#version 150" if NULL.  Only override if your GL version doesn't handle this
// GLSL version. Keep NULL if unsure!
...
extern(C) bool     ImGui_ImplGlfw_InitForVulkan(
  GLFWwindow* window, bool install_callbacks
);
...
extern(C) void     ImGui_ImplGlfw_MouseButtonCallback(
  GLFWwindow* window, int button, int action, int mods
);
extern(C) void     ImGui_ImplGlfw_ScrollCallback(
  GLFWwindow* window, double xoffset, double yoffset
);
extern(C) void     ImGui_ImplGlfw_KeyCallback(
  GLFWwindow* window, int key, int scancode, int action, int mods
);
extern(C) void     ImGui_ImplGlfw_CharCallback(
  GLFWwindow* window, uint c
);
