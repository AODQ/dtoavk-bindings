module dtoavkbindings.cimgui_gl;
...
//  context won't fail if the function is a null pointer.
...
import core.stdc.stdint, core.stdc.stdint, core.stdc.stdarg, core.stdc.stdio;
...
// #if defined(GL_CLIP_ORIGIN) && !defined(__APPLE__)
    GLenum last_clip_origin = 0; glGetIntegerv(GL_CLIP_ORIGIN, cast(GLint*)&last_clip_origin); // Support for GL 4.5's glClipControl(GL_UPPER_LEFT)
            ...
            else
            {
                ...
                if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
                {
                    ...
                    if (clip_origin_lower_left)
                        ...
                        glScissor(cast(int)clip_rect.x, cast(int)clip_rect.y, cast(int)clip_rect.z, cast(int)clip_rect.w); // Support for GL 4.5 rarely used glClipControl(GL_UPPER_LEFT)
...
static bool CheckProgram(GLuint handle, const (char)* desc)
{
    ...
    if (log_length > 1)
    {
      printf("FUCK THE PROGRAMF FUCKED\n");
      printf("FUCK THE PROGRAMF FUCKED\n");
      printf("FUCK THE PROGRAMF FUCKED\n");
      printf("FUCK THE PROGRAMF FUCKED\n");
        // ImVector<char> buf;
